            ...
            <tbody>
              {events.length > 0 ? events.map((e, i) => (
                <tr key={i} className="hover:bg-sky-50/50 transition-colors duration-200 group">
                  ...
                  <td className="px-4 py-3 border font-medium">{e.event_code}</td>
                  <td className="px-4 py-3 border">{e.event_name}</td>
                  <td className="px-4 py-3 border">{e.arrived}</td>
                  <td className="px-4 py-3 border">{e.departed}</td>
                  <td className="px-4 py-3 border font-bold text-green-600">{e.present}</td>
                </tr>
              )) : (
                <tr>
                  <td colSpan="6" className="text-center py-10 text-gray-500">No approved events found.</td>
                </tr>
              )}
            </tbody>
          </table>
              ...
              <tbody>
                {entries.length > 0 ? entries.map((v, i) => (
                  <tr key={i} className="hover:bg-sky-50/50 transition-colors duration-200 group">
                    ...
                    <td className="px-4 py-3 border font-mono text-sm">{v.visitor_code}</td>
                    <td className="px-4 py-3 border">{v.name}</td>
                    <td className="px-4 py-3 border text-sm text-gray-600">{v.checkin_time || "---"}</td>
                    <td className="px-4 py-3 border text-sm text-gray-600">{v.checkout_time || "---"}</td>
                    <td className="px-4 py-3 border text-center">
                      {!v.checkin_time ? (
                        <button 
                  ...
                  <tr>
                    <td colSpan="6" className="text-center py-10 text-gray-500">No entries found for this event.</td>
                  </tr>
                )}
              </tbody>
            </table>
