...
def create_tables():
    ...
    cursor = db.cursor()
    cursor.execute("CREATE DATABASE IF NOT EXISTS event_dbone")
...
"""
    ...
    def alter_booking_details_table(cursor):
        try:
            ...
            print("⚠️ Alter booking error:", str(e))
...
"""
    ...
    def alter_users_table(cursor):
        try:
            ...
            for col_name, col_type in columns_to_add:
                cursor.execute("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'users' AND COLUMN_NAME = %s", (col_name,))
                if not cursor.fetchone():
