...
def complete_event_creation():
    ...
    try:
        ...
        if banner:
            folder, relative_cat, file_type = get_file_folder(banner)
            filename = secure_filename(banner.filename)
            save_path = os.path.join(folder, filename)
            db_path = f"/uploads/{relative_cat}/{filename}"
            banner.save(save_path)
            cursor.execute("INSERT INTO event_files (event_id, file_name, file_path, file_type) VALUES (%s, %s, %s, %s)", (event_id, filename, db_path, file_type))
    ...
    try:
        ...
        if organizer:
            ...
            LEFT JOIN event_files f 
                ON e.id = f.event_id AND (f.file_type = 'banner' OR f.file_type = 'image' OR f.file_type = 'video')
    ...
    try:
        ...
        for item in todo_items:
            ...
            """, (
                ...
                int(item.get('complete_percent', 0)),
                item.get('remarks', '')
            ))
